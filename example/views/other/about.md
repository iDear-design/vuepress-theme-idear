...
随着不断有用户过来询问：为什么 `vuepress-theme-ido@0.x` 不能在 `vuepress@1.x` 上使用？本来只是打算对 `vuepress-theme-ido@0.x` 进行简单的bug修复的我，终究还是忍不住，开始了 `vuepress-theme-ido@1.x` 的开发。又是在一个寂静的凌晨两点半（晚上就是出活快），我默默地开始了。
...
`vuepress-theme-ido@1.x` 的开发中，更加深刻地明白了模块化和组件化编程的重要性，如果当初没有把一些功能进行封装，而是直接简单的复制，这次升级也不会这么顺利。模块拆分的越细，使用就会越灵活。
