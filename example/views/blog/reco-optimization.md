...
- 与hexo不一样，正常的主题文件会被安装到`node_modules`文件夹中。如果之前`vuepress-theme-iu`安装正确的话，可以在`node_modules`文件夹中找到改文件。所以如果不把主题文件放到本地，我们每次执行**npm install**的时候，新的文件就会把我们魔改过的文件覆盖掉。
...
module.exports = [
    ...
    /* 音乐插件   
    ["@iu-design/vuepress-plugin-bgm-player",
          {
            audios: [
              // 网络文件示例
              {
...
1. [bgm-player](https://github.com/iu-design/vuepress-plugin-bgm-player)，一款简洁易用的音乐插件，优势是好看，其他一无是处了:slightly_smiling_face:
...
通过全局搜索，终于找到在源码文件`\xxx\blog\node_modules\vuepress-theme-iu\components\SidebarGroup.vue`中发现问题，罪魁祸首就是这个`opacity 0.5`，致此，将其改成1即可，虽然简单，但是对于有页面强迫症的我，确实是极大的舒适。
...
vuepress-theme-iu：iu-design.maxiaoqu.com/vuepress-theme-iu/
